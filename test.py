...
cave_bg = pygame.image.load(resource_path("pics/cave_entrance_background.png")).convert()
cave_bg = pygame.transform.scale(cave_bg, (WIDTH, HEIGHT))

guardian1_img = pygame.image.load(resource_path("pics/cave_entrance_guardian_1.png"))
guardian1_img = pygame.transform.scale(guardian1_img, (190, 190))
guardian2_img = pygame.image.load(resource_path("pics/cave_entrance_guardian_2.png"))
guardian2_img = pygame.transform.scale(guardian2_img, (190, 190))

guardian1_hp = 10
guardian2_hp = 7

guardian1_rect = guardian1_img.get_rect(midtop=(WIDTH//2 - 150, 40))
guardian2_rect = guardian2_img.get_rect(midtop=(WIDTH//2 + 150, 40))
...
# --- Asset loading ---
frog_img = pygame.transform.scale(pygame.image.load(resource_path("pics/frog.png")), (180, 180))
...
runes = []
floating_texts = []
haste_timer = hex_timer = invisible_timer = boss_outro_timer = boss_intro_timer = 0
...
def draw_hp_bar(x,y,hp,max_hp):
    ...
    pygame.draw.rect(screen,(60,0,0),(x,y,120,12))
    pygame.draw.rect(screen,(220,0,0),(x,y,120*(hp/max_hp),12))
...
while running:
    ...
    if game_state == HERO_SELECT:
        ...
        text1 = font.render(
            "You have arrived at the cave entrance, but it looks to be guarded.",
            True, (200, 200, 200))

        text2 = font.render(
            "You prepare your weapon and charge ahead.",
            True, (200, 200, 200))

        screen.blit(text1, (WIDTH // 2 - text1.get_width() // 2, HEIGHT // 2 - 40))
        screen.blit(text2, (WIDTH // 2 - text2.get_width() // 2, HEIGHT // 2 + 10))

        if pygame.time.get_ticks() - boss_intro_timer > 4000:
        ...
        # draw bosses
        screen.blit(guardian1_img, guardian1_rect)
        screen.blit(guardian2_img, guardian2_rect)

        draw_hp_bar(guardian1_rect.x, guardian1_rect.bottom + 5, guardian1_hp, 10)
        draw_hp_bar(guardian2_rect.x, guardian2_rect.bottom + 5, guardian2_hp, 7)
        ...
        if guardian1_hp <= 0 and guardian2_hp <= 0:
            game_state = BOSS_OUTRO
            boss_outro_timer = pygame.time.get_ticks()

        # timers

        if haste_timer and pygame.time.get_ticks() - haste_timer > 5000:
            player_speed = base_speed
            haste_timer = 0

        if hex_timer and pygame.time.get_ticks() - hex_timer > 5000:
            player_speed = base_speed
            player_image = original_player_img
            hex_timer = 0
    ...
    elif game_state == BOSS_OUTRO:

        screen.fill((0, 0, 0))

        text1 = font.render(
            "You quickly defeated the local inhabitants.",
            True, (200, 200, 200))

        text2 = font.render(
            "You feel the adrenaline rush (+1 speed)",
            True, (200, 200, 200))

        screen.blit(text1, (WIDTH // 2 - text1.get_width() // 2, HEIGHT // 2 - 40))
        screen.blit(text2, (WIDTH // 2 - text2.get_width() // 2, HEIGHT // 2 + 10))

        if pygame.time.get_ticks() - boss_outro_timer > 4000:
            player_bonus_speed += 1

            reset_game(False)

            player_speed = base_speed + player_bonus_speed
        ...
        if game_state == PLAYING:
            ...
            if gold >= 100 and game_state == PLAYING:
